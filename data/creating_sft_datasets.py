"""Create SFT datasets from Dolci-Think-SFT-7B using predefined source groups."""
...
DOLCI_DATASET = "allenai/Dolci-Think-SFT-7B"
TOKENIZER_NAME = "facebook/MobileLLM-R1-140M-base"
OUTPUT_ROOT = Path("data")
MAX_TOKENS = 4096
PER_DATASET_TOKEN_BUDGET = 60_000_000
...
SFT_SECTION_TO_DATASET = {
    "MATH & STEM DATA": "Shekswess/tiny-think-sft-math-n-stem",
}
SFT_MATH_N_STEM_DATASET = SFT_SECTION_TO_DATASET["MATH & STEM DATA"]
SFT_MATH_N_STEM_PRIMARY_SOURCE = (
    "saumyamalik/OpenThoughts3-full-filtered-science-decontam-v2"
)
SFT_MATH_N_STEM_MIN_OTHER_SHARE = 0.65  # Minimum token share from non-primary sources.
SFT_MATH_N_STEM_MAX_PRIMARY_SHARE = 0.35  # Maximum token share for the primary source.
SFT_MATH_N_STEM_DIVERSITY_POLICY = "strict"  # "strict" (raise) or "adaptive" (shrink).
INCLUDE_GSM8K_THINK_SFT = True
REBUILD_GSM8K_THINK_SFT = True
GSM8K_TARGET_GROUP = "Shekswess/tiny-think-sft-math-n-stem"
GSM8K_DATASET_NAME = "openai/gsm8k"
GSM8K_DATASET_CONFIG = "main"
GSM8K_SPLIT_NAME = "train"
GSM8K_OUTPUT_DATASET_NAME = "Shekswess/gsm8k_think_sft"
GSM8K_DATASET_SOURCE = "gsm8k"
GSM8K_OUTPUT_ROOT = Path("data/datasets")
REFERENCE_DATASET_NAME = "Shekswess/tiny-think-sft-math-n-stem"
...
def load_sft_dataset_groups(source_file: Path) -> Dict[str, Tuple[str, ...]]:
    """
    Load SFT dataset groups from the cleaned source values file.
    ...
    groups: Dict[str, List[str]] = {
        dataset_name: [] for dataset_name in SFT_SECTION_TO_DATASET.values()
    }
    ...
    for raw_line in source_file.read_text(encoding="utf-8").splitlines():
        ...
        if line.endswith(":"):
            header = line[:-1].strip()
            if header not in SFT_SECTION_TO_DATASET:
                raise RuntimeError(f"Unknown SFT section header: {header!r}")
        ...
        if line.startswith("- "):
            ...
            source = line[2:].strip()
            if source:
                dataset_name = SFT_SECTION_TO_DATASET[current_section]
    ...
    for dataset_name, sources in groups.items():
        ...
        if not unique_sources:
            raise RuntimeError(
                f"SFT dataset group {dataset_name!r} has no sources in {source_file}."
            )
        dataset_groups[dataset_name] = tuple(unique_sources)
    return dataset_groups


def load_dpo_source_values(source_file: Path) -> Set[str]:
    """
    Load DPO source values from the cleaned DPO source values file.

    Args:
        source_file: Path to the cleaned DPO source values file.

    Returns:
        Set of DPO source strings.
    ...
    if not sources:
        raise RuntimeError(f"No sources found in DPO source file: {source_file}")
    return sources
...
def collect_sft_source_values(dataset_groups: Dict[str, Sequence[str]]) -> Set[str]:
    """
    Collect all source values from SFT dataset groups.

    Args:
        dataset_groups: Mapping of dataset name to source sequences.

    Returns:
        Set of all SFT source values.
    """
    sources: Set[str] = set()
...
def count_tokens(
    ...
    """
    Count tokens for a message list using the provided tokenizer.

    Args:
        messages: List of message dicts from the dataset.
        tokenizer: Tokenizer used for token counting.

    Returns:
        Token count for the serialized conversation.
    """
...
def add_token_counts(
    ...
    Returns:
        Dataset with an added `token_count` column.
    """
    def _map(batch: Dict[str, List]) -> Dict[str, List[int]]:
        global _TOKENIZER
        if _TOKENIZER is None:
            _TOKENIZER = tokenizer
        counts = [count_tokens(messages, _TOKENIZER) for messages in batch["messages"]]
...
def parse_gsm8k_answer(answer: str) -> Tuple[str, str]:
    """
    Parse a GSM8K answer into reasoning and final answer.

    Args:
        answer: Raw GSM8K answer string containing a "####" delimiter.

    Returns:
        Tuple of (reasoning, final_answer).

    Raises:
        ValueError: If the answer is malformed or missing the delimiter.
    """
    if not isinstance(answer, str) or not answer.strip():
        raise ValueError("Answer must be a non-empty string.")

    if "####" not in answer:
        raise ValueError("Answer is missing the '####' delimiter.")
    ...
    if not reasoning or not final:
        raise ValueError("Answer parsing produced empty reasoning or final answer.")

    return reasoning, final


def format_gsm8k_messages(question: str, answer: str) -> List[Dict[str, str]]:
    """
    Format GSM8K question/answer into chat messages.

    Args:
        question: GSM8K question text.
        answer: GSM8K answer text containing the reasoning and final answer.

    Returns:
        List of role/content message dicts.

    Raises:
        ValueError: If the question is empty or the answer is malformed.
    """
...
def build_gsm8k_think_dataset(raw_dataset: Dataset) -> Dataset:
    """
    Build the GSM8K think-style dataset with messages and source.

    Args:
        raw_dataset: The raw GSM8K dataset.

    Returns:
        Dataset with messages and dataset_source columns.
    ...
    if not required.issubset(columns):
        raise RuntimeError(
            f"Expected GSM8K columns {sorted(required)}, found {sorted(columns)}."
        )

    def _map(batch: Dict[str, List[str]]) -> Dict[str, List]:
        messages: List[List[Dict[str, str]]] = []
        sources: List[str] = []
        for question, answer in zip(batch["question"], batch["answer"]):
            messages.append(format_gsm8k_messages(question, answer))
            sources.append(GSM8K_DATASET_SOURCE)
        return {"messages": messages, "dataset_source": sources}

    dataset = raw_dataset.map(_map, batched=True, batch_size=256, num_proc=NUM_PROC)
    drop_columns = [
        col for col in dataset.column_names if col not in {"messages", "dataset_source"}
    ]
    if drop_columns:
...
def normalize_columns(dataset: Dataset, source_column: str) -> Dataset:
    ...
    Returns:
        Dataset with `dataset_source` column name and required columns only.
    """
    if source_column != "dataset_source":
        dataset = dataset.rename_column(source_column, "dataset_source")

    keep = {"messages", "dataset_source", "token_count"}
...
def output_path_for_gsm8k_dataset(dataset_name: str) -> Path:
    """
    Create a filesystem path for GSM8K dataset artifacts.

    Args:
        dataset_name: Hugging Face dataset name (owner/name).

    Returns:
        Filesystem path under the data/datasets directory.
    """
    safe_name = dataset_name.replace("/", "__")
    return GSM8K_OUTPUT_ROOT / safe_name


def create_gsm8k_think_sft_dataset(
    save_to_disk: bool = True, push_to_hub: bool = True
) -> tuple[Dataset, int]:
    """
    Create the GSM8K think-style SFT dataset.
    ...
    Returns:
        Tuple of (dataset, total_tokens).

    Raises:
        RuntimeError: If any step of dataset creation or upload fails.
    """
    try:
        raw_dataset = load_dataset(
            GSM8K_DATASET_NAME, GSM8K_DATASET_CONFIG, split=GSM8K_SPLIT_NAME
        )
    except Exception as exc:
        logger.exception("Failed loading dataset {}", GSM8K_DATASET_NAME)
        raise RuntimeError(f"Failed loading dataset {GSM8K_DATASET_NAME}") from exc

    logger.info(
        "Loaded GSM8K dataset {} ({} split: {} rows)",
        GSM8K_DATASET_NAME,
        GSM8K_SPLIT_NAME,
        raw_dataset.num_rows,
    )

    try:
        dataset = build_gsm8k_think_dataset(raw_dataset)
    except Exception as exc:
        logger.exception("Failed building GSM8K think messages")
        raise RuntimeError("Failed building GSM8K think messages") from exc

    try:
        tokenizer = AutoTokenizer.from_pretrained(TOKENIZER_NAME)
    except Exception as exc:
        logger.exception("Failed loading tokenizer {}", TOKENIZER_NAME)
        raise RuntimeError(f"Failed loading tokenizer {TOKENIZER_NAME}") from exc

    if tokenizer.pad_token is None and tokenizer.eos_token is not None:
        tokenizer.pad_token = tokenizer.eos_token

    try:
        dataset = add_token_counts(dataset, tokenizer)
    except Exception as exc:
        logger.exception("Failed computing token counts")
        raise RuntimeError("Failed computing token counts") from exc

    try:
        dataset = dataset.filter(
            lambda row: row["token_count"] <= MAX_TOKENS,
            num_proc=NUM_PROC,
        )
    except Exception as exc:
        logger.exception("Failed applying token cutoff")
        raise RuntimeError("Failed applying token cutoff") from exc

    logger.info(
        "Remaining rows after {} token cutoff: {}",
        MAX_TOKENS,
        dataset.num_rows,
    )

    total_tokens = int(sum(dataset["token_count"]))
    logger.info("Total tokens in GSM8K think SFT dataset: {}", total_tokens)

    dataset = dataset.shuffle(seed=SHUFFLE_SEED)

    if save_to_disk:
        output_path = output_path_for_gsm8k_dataset(GSM8K_OUTPUT_DATASET_NAME)
        try:
            output_path.mkdir(parents=True, exist_ok=True)
            dataset.save_to_disk(str(output_path))
        except Exception as exc:
            logger.exception("Failed saving GSM8K dataset to {}", output_path)
            raise RuntimeError(
                f"Failed saving GSM8K dataset to {output_path}"
            ) from exc

        logger.info("Saved GSM8K dataset to {}", output_path)

    if push_to_hub:
        try:
            dataset.push_to_hub(GSM8K_OUTPUT_DATASET_NAME)
        except Exception as exc:
            logger.exception(
                "Failed pushing GSM8K dataset to Hub: {}",
                GSM8K_OUTPUT_DATASET_NAME,
            )
            raise RuntimeError(
                f"Failed pushing {GSM8K_OUTPUT_DATASET_NAME} to the Hub"
            ) from exc

        logger.info("Pushed GSM8K dataset to the Hub: {}", GSM8K_OUTPUT_DATASET_NAME)
...
def allocate_token_budgets(
    ...
    if manual_budgets is not None:
        ...
        if total_manual != total_budget:
            logger.warning(
                f"Manual token budgets sum to {total_manual}, "
                f"but total budget is {total_budget}."
            )
...
def allocate_source_budgets(
    ...
    Args:
        source_shares: Mapping of source to token share (sums to ~1.0).
        total_budget: Total tokens to allocate.

    Returns:
        Mapping of source to token budget (sum equals total_budget).
    """
...
def downsample_by_source_budgets(
    ...
    Args:
        ...
        source_budgets: Token budget per source.
        seed: Random seed for shuffling.
        source_column: Column containing dataset source values.
        strict: Whether to raise if a source lacks tokens.
    ...
    for idx, (source, budget) in enumerate(source_budgets.items()):
        ...
        selected, kept = select_rows_by_token_budget(subset, budget)
        if kept < budget:
            message = (
...
def select_rows_by_token_budget(
    dataset: Dataset, token_budget: int
) -> tuple[Dataset, int]:
    """
    Select rows up to a token budget without reshuffling.
    ...
    Returns:
        Tuple of (selected dataset, total tokens kept).
    """
    if dataset.num_rows == 0 or token_budget <= 0:
        return dataset.select([]), 0

    counts = dataset["token_count"]
    total = 0
    keep = 0
    for count in counts:
...
def downsample_to_token_budget(
    ...
    """
    Downsample a dataset to fit within a token budget, prioritizing overlap sources.
    ...
    if not preferred_sources:
        ...
        return select_rows_by_token_budget(shuffled, token_budget)
    ...
    preferred, preferred_tokens = select_rows_by_token_budget(preferred, token_budget)
    remaining_budget = token_budget - preferred_tokens
    remaining_tokens = 0
    if remaining_budget > 0:
        remaining, remaining_tokens = select_rows_by_token_budget(
...
def load_gsm8k_think_sft_dataset(rebuild: bool = True) -> Dataset:
    """
    Load or build the GSM8K think-style SFT dataset.

    Args:
        rebuild: Whether to rebuild the dataset even if it exists on disk.

    Returns:
        GSM8K think-style dataset with messages and token counts.

    Raises:
        RuntimeError: If the dataset cannot be built or loaded.
    """
    output_path = output_path_for_gsm8k_dataset(GSM8K_OUTPUT_DATASET_NAME)
    if not rebuild and output_path.exists():
        logger.info("Loading GSM8K think SFT dataset from {}", output_path)
        return load_from_disk(str(output_path))

    logger.info("Building GSM8K think SFT dataset (rebuild={})", rebuild)
    dataset, total_tokens = create_gsm8k_think_sft_dataset(
        save_to_disk=True, push_to_hub=True
    )
    logger.info("GSM8K think SFT tokens: {}", total_tokens)
    return dataset


def main() -> None:
    """
    Create grouped SFT datasets, save them, and push to the Hub.

    Raises:
        RuntimeError: If the source dataset cannot be loaded.
    """
    dataset_groups = load_sft_dataset_groups(SFT_SOURCE_FILE)
    if set(dataset_groups) != {SFT_MATH_N_STEM_DATASET}:
        raise RuntimeError(
            "SFT dataset groups must contain only the math+stem dataset. "
            f"Found: {sorted(dataset_groups)}"
        )
    sft_sources = collect_sft_source_values(dataset_groups)
    dpo_sources = load_dpo_source_values(DPO_SOURCE_FILE)
    ...
    try:
        dataset = load_dataset(DOLCI_DATASET, split="train", streaming=False)
    except Exception as exc:
        logger.exception("Failed loading dataset %s", DOLCI_DATASET)
        raise RuntimeError(f"Failed loading dataset {DOLCI_DATASET}") from exc

    source_column = detect_source_column(getattr(dataset, "column_names", []))
    normalized_source_column = "dataset_source"
    tokenizer = AutoTokenizer.from_pretrained(TOKENIZER_NAME, use_fast=True)
    logger.info(f"Using num_proc={NUM_PROC}")
    available_tokens: Dict[str, int] = {}
    ...
    for target_name, sources in dataset_groups.items():
        ...
        logger.info(f"Building {target_name} from {len(source_set)} sources")

        filtered = dataset.filter(
            lambda row: row[source_column] in source_set,
            num_proc=NUM_PROC,
        )
        logger.info(f"{filtered.num_rows} examples selected for {target_name}")
        ...
        logger.info(
            f"{filtered.num_rows} examples remain after {MAX_TOKENS} token cutoff"
        )
        filtered = normalize_columns(filtered, source_column)
        if INCLUDE_GSM8K_THINK_SFT and target_name == GSM8K_TARGET_GROUP:
            gsm8k_dataset = load_gsm8k_think_sft_dataset(
                rebuild=REBUILD_GSM8K_THINK_SFT
            )
            missing_columns = {
                "messages",
                "dataset_source",
                "token_count",
            } - set(gsm8k_dataset.column_names)
            if missing_columns:
                raise RuntimeError(
                    "GSM8K think SFT dataset missing required columns: "
                    f"{sorted(missing_columns)}"
                )
            filtered = concatenate_datasets([filtered, gsm8k_dataset])
            logger.info(
                "Added GSM8K think SFT to {} (rows now: {})",
                target_name,
                filtered.num_rows,
            )
        if MAX_ROWS is not None and filtered.num_rows > MAX_ROWS:
            filtered = filtered.shuffle(seed=SHUFFLE_SEED).select(range(MAX_ROWS))
            logger.info(f"Downsampled to {MAX_ROWS} examples for {target_name}")
        total_tokens = sum(filtered["token_count"])
        available_tokens[target_name] = total_tokens
        prepared[target_name] = filtered
        logger.info(f"Available tokens for {target_name}: {total_tokens}")
    ...
    logger.info(f"Per-dataset token budget target: {PER_DATASET_TOKEN_BUDGET}")
    logger.info(f"Total token budget: {total_budget}")
    logger.info(f"Token budget strategy: {TOKEN_BUDGET_STRATEGY}")
    logger.info(
        "Math+STEM diversity: primary_source={}, min_other_share={:.0%}, "
        "max_primary_share={:.0%}, policy={}",
        SFT_MATH_N_STEM_PRIMARY_SOURCE,
        SFT_MATH_N_STEM_MIN_OTHER_SHARE,
        SFT_MATH_N_STEM_MAX_PRIMARY_SHARE,
        SFT_MATH_N_STEM_DIVERSITY_POLICY,
    )
    for name, budget in token_budgets.items():
        logger.info(f"Token budget for {name}: {budget}")

    token_totals: Dict[str, int] = {}
    for target_name, filtered in prepared.items():
        budget = token_budgets[target_name]
        if target_name != SFT_MATH_N_STEM_DATASET:
            raise RuntimeError(
                "Only the math+stem dataset is supported for SFT creation."
            )
        ...
        if USE_REFERENCE_PROPORTIONS:
            ...
            filtered, kept_tokens = downsample_with_min_other_share(
                ...
                budget,
                seed=SHUFFLE_SEED,
                primary_source=SFT_MATH_N_STEM_PRIMARY_SOURCE,
                min_other_share=SFT_MATH_N_STEM_MIN_OTHER_SHARE,
                max_primary_share=SFT_MATH_N_STEM_MAX_PRIMARY_SHARE,
                policy=SFT_MATH_N_STEM_DIVERSITY_POLICY,
                preferred_sources=overlap_sources,
                source_column=normalized_source_column,
            )
        if kept_tokens < budget and available_tokens[target_name] < budget:
            logger.warning(
                f"{target_name} has {available_tokens[target_name]} available tokens "
                f"(< budget {budget}); using all available tokens."
            )
        elif kept_tokens < budget:
            logger.warning(
                f"{target_name} selected {kept_tokens} tokens (< budget {budget})."
            )
        token_totals[target_name] = kept_tokens
        logger.info(f"Selected tokens for {target_name}: {kept_tokens}")
        prepared[target_name] = filtered
        filtered = normalize_columns(filtered, normalized_source_column)

        output_path = output_path_for_dataset(target_name)
        output_path.mkdir(parents=True, exist_ok=True)
        filtered.save_to_disk(str(output_path))
        logger.info(f"Saved {target_name} to {output_path}")

        try:
            filtered.push_to_hub(target_name)
            logger.info(f"Pushed {target_name} to the Hub.")
        except Exception as exc:
            logger.exception("Failed pushing %s to the Hub.", target_name)
    ...
    if token_totals:
        ...
        logger.info(f"Total tokens across datasets: {sum(token_totals.values())}")
